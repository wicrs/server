...
impl Actor for MessageServerOther {
    type Context = Context<Self>;

    fn stopping(&mut self, _: &mut Self::Context) -> Running {
        /*for (hc_id, writer) in self.index_writers.try().await {
            let _ = writer.commit();
...
impl Handler<SearchMessageIndex> for MessageServerOther {
...
impl Handler<NewMessageForIndex> for MessageServerOther {
...
impl MessageServer {
    ...
    fn setup_index(&mut self, hub_id: &ID, channel_id: &ID) -> Result {
        ...
        if log_path.is_file() {
            ...
            let hub = serde_json::from_str::<Hub>(&json).map_err(|_| DataError::Deserialize)?;
            if let Some(channel) = hub.channels.get(channel_id) {
                let messages: Vec<channel::Message> = channel.get_all_messages_from(&last_id); //channel.get_all_messages_from(&last_id);
                let last_id = if let Some(last) = messages.last() {
                    Some(last.id.clone())
                } else {
                    None
